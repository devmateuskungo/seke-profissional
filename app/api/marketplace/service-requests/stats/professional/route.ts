...
/** GET /api/marketplace/service-requests/stats/professional — proxy para API externa */
export async function GET(request: NextRequest) {
  try {
    const auth = getAuthorizationHeader(request)
    if (!auth.ok) return auth.response

    const endpoint = `${getApiBaseUrl()}/marketplace/service-requests/stats/professional`
    ...
    if (!res.ok) {
      const message =
        ...
        "Não foi possível carregar as métricas do profissional."
