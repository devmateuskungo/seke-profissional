...
/** GET /api/auth/perfil - Retorna dados completos de perfil do utilizador atual */
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(perfilEndpoint, {
      method: "GET",
      headers: {
    ...
    if (!res.ok) {
      const message =
        ...
        "Falha ao obter dados de perfil."
    ...
    return NextResponse.json(
      { message: "Erro interno ao obter dados de perfil." } satisfies ApiErrorResponse,
      { status: 500 }
    )
  }
}
